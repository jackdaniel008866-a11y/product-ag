...
export default function GlobalSearch({ initiatives, onSelect }: GlobalSearchProps) {
  ...
  const renderHighlightedText = (text: string | undefined, matches: ReadonlyArray<{ indices: ReadonlyArray<[number, number]> }> | undefined) => {
    ...
    mergedIntervals.forEach(([start, end], i) => {
      ...
      parts.push(<span key={`m-${i}`} className="bg-amber-200/50 text-indigo-900 font-bold px-0.5 rounded">{text.slice(start, end + 1)}</span>);
      lastIndex = end + 1;
    });
    ...
    <div className="flex-1 max-w-xl px-12 relative hidden md:block" ref={containerRef}>
      <div className="relative group">
        ...
        {isOpen && debouncedQuery && (
          <div className="absolute top-10 left-12 right-12 bg-white border border-slate-200 rounded-xl shadow-xl shadow-slate-200/50 overflow-hidden z-[100] animate-in slide-in-from-top-2 duration-200">
            {results.length === 0 ? (
              <div className="p-4 flex items-center justify-center text-slate-500 text-sm py-8 flex-col space-y-2">
                ...
                <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Try adjusting your spelling</p>
              </div>
            ) : (
              <ul className="py-1.5">
                  ...
                  return (
                    <li 
                      ...
                      className={clsx(
                        ...
                        index === selectedIndex ? "bg-slate-50 border-teal-500" : "hover:bg-slate-50 border-transparent"
                      )}
                    ...
                    >
                      <div className="flex items-center justify-between mb-0.5">
                        ...
                        <div className="flex items-center space-x-1.5 shrink-0 ml-3">
                          <span className="text-[9px] uppercase font-bold tracking-wider text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">
                            {init.type}
                          </span>
                          <span className={clsx(
                            "text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded",
                            init.status === 'Active' ? 'text-emerald-700 bg-emerald-50' : 
                            init.status === 'Blocked' ? 'text-red-700 bg-red-50' : 
                      ...
                      {descMatch && !titleMatch ? (
                        <div className="flex items-start text-xs text-slate-500 mt-1 line-clamp-1 opacity-90">
            ...
            <div className="bg-slate-50 p-2 border-t border-slate-100 flex justify-between items-center px-4 w-full">
               <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Esc to close</span>
               <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400 flex items-center space-x-2">
